...
const SUBMIT_TOOL = {
  ...
  input_schema: {
    ...
    required: ['links', 'keywords', 'palette'],
  },
};
...
app.post('/api/auto-research', async (req, res) => {
  try {
    ...
    const { topic, brief, category } = req.body;
    ...
    const userText =
      ...
      `Brief: ${brief?.trim() || '(none provided)'}\n\n` +
      `Search the web for current design inspiration, competitor/reference examples, and trends ` +
      `relevant to this category. Then call submit_research_pack exactly once with: 4-8 reference ` +
      `links you actually found via web_search (each with a 1-line description of what's useful ` +
      `about it), 4-6 moodboard keywords/descriptors, and a suggested starting color palette of 4-6 ` +
      `hex codes with a short reasoning for each. Do not invent URLs — only include links returned by web_search.`;
    ...
    res.json({ pack: submitBlock.input });
  } catch (err) {
    console.error(err);
